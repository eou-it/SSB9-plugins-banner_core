...
//    private static final Logger log = Logger.getLogger(getClass())
    ...
    public void setUp() {
        ...
        tearDownTestFoo(true)
        // tearDown should take care of this -- will really only be effective once we stop managing
    ...
    void testSaveInvalid() {
        try {
            ...
            assertNotNull returnMap.underlyingErrorMessage
            // this is the underlying exception's message (which is not appropriate to display to a user)
        }
    }
    ...
    void testUpdate() {
        ...
        sleep 1000
        // would not really be needed, except that 'Foo' is marked (artificially, and incorrectly) as needing a refresh() immediately
    ...
    //
    @Ignore
    // TODO: Remove flushImmediate option on ServiceBase methods, or correctly handle a delayed flush within ServiceBase
...
// -------------------------------------- Supporting Methods -------------------------------------------
    ...
    @Test
    void testFacultyQueryViewForTesting() {
        FacultyScheduleQueryViewForTesting f = new FacultyScheduleQueryViewForTesting()
        f.toString()
    }
}