...
class BannerSamlLogoutFilterTest extends BaseIntegrationTestCase {
    ...
    public void testAuthenticateOracleUser() {
        // todo: logout filter needs to be called. Creating a dummy request object will not test the filter
        LogoutRequest request = initializeLogout("9BE22914996B2516E040007F01006516")
    ...
    public LogoutRequest initializeLogout(String UDCID){

        SAMLAuthenticationToken token = initialize(UDCID)
        ...
        request.setNotOnOrAfter(new DateTime().plusSeconds(120))
        ...
        index.setSessionIndex("7635c467-c47b-4f1d-81dd-7f86b2f5b00d");
        request.getSessionIndexes().add(index);

        if (request.getSessionIndexes().size() == 0) {
            assert("No session indexes to logout user for were found");
        }
        ...
        nameID.setValue(UDCID);
        request.setNameID(nameID);

        SAMLObjectBuilder<Issuer> issuerSAMLObjectBuilder= (SAMLObjectBuilder<Issuer>) builderFactory.getBuilder(Issuer.DEFAULT_ELEMENT_NAME);
        Issuer issuer=issuerSAMLObjectBuilder.buildObject();
        SAMLCredential samlCredential = authentication.getSAMLCredential()
        issuer.setValue(samlCredential.nameID.getValue())
        request.setIssuer(issuer);


        SAMLObjectBuilder<Response> responseSAMLObjectBuilder = (SAMLObjectBuilder<Response>) builderFactory.getBuilder(Response.DEFAULT_ELEMENT_NAME);
        ...
        messageContext.setOutboundMessage(request)
